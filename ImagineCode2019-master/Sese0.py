...
for j in list(range(len(obj_fin))):
    ...
    while (recibido1 == False):
        ...
        print("Nº a transportar :" + num_fin[j])
        ...
        if ((lang == "es" and num_es[int(num_fin[j])-1] == output.num) or (lang == "en" and num_en[int(num_fin[j])-1] == output.num)) and producto == output.obj and output.rtype == 1 and pos_ini[i].lower() == output.box.lower():
            recibido1 = True
        elif output.rtype == 4:
            recibido1 = True
        else:
            # Error de cantidad
            if not ((lang == "es" and num_es[int(num_fin[j])-1] == output.num) or (lang == "en" and num_en[int(num_fin[j])-1] == output.num)):
                print("Error de cantidad")
            # Error de producto
            if not producto == output.obj:
                print("Error de producto")
            # Error de caja
            if not pos_ini[i].lower() == output.box.lower():
                print("Error de caja")
    ...
    while (recibido2 == False):
        ...
        if pos_fin[j].lower() == output.box.lower() and output.rtype == 2 and ((lang == "es" and num_es[int(num_fin[j])-1] == output.num) or (lang == "en" and num_en[int(num_fin[j])-1] == output.num)):
            recibido2 = True
        elif output.rtype == 4:
            recibido2 = True
        else:
            if pos_fin[j].lower() == output.box.lower():
                print("Error de caja")
            if output.rtype == 2:
                print("Formato de frase incorrecto")
            if (lang == "es" and num_es[int(num_fin[j])-1] == output.num) or (lang == "en" and num_en[int(num_fin[j])-1] == output.num):
